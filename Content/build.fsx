...
let getGcloudProject() =
    runTool (gcloudTool ()) "config get-value project -q" __SOURCE_DIRECTORY__
...
let deployExists appName =
    let result = runTool "kubectl" "get deploy" __SOURCE_DIRECTORY__
...
let updateKubernetesDeploy appName dockerTag =
    ...
    runTool "kubectl" updateArgs __SOURCE_DIRECTORY__ |> ignore
...
let createAndExposeKubernetesDeploy appName dockerTag port =
    ...
    runTool "kubectl" deployArgs __SOURCE_DIRECTORY__ |> ignore

    let exposeArgs = sprintf "expose deployment %s --type=LoadBalancer --port 80 --target-port %i" appName port
    runTool "kubectl" exposeArgs __SOURCE_DIRECTORY__ |> ignore
//#endif
...
//#if (js-deps == "npm")
    printfn "Npm version:"
    runTool npmTool "--version"  __SOURCE_DIRECTORY__ |> ignore
    runTool npmTool "install" __SOURCE_DIRECTORY__ |> ignore
//#else
...
//#if (js-deps == "npm")
    runTool npxTool "webpack-cli -p" __SOURCE_DIRECTORY__ |> ignore
//#else
    runTool yarnTool "webpack-cli -p" __SOURCE_DIRECTORY__ |> ignore
//#endif
)
...
Target.create "Run" (fun _ ->
    ...
    let client = async {
//#if (js-deps == "npm")
        runTool npxTool "webpack-dev-server" __SOURCE_DIRECTORY__ |> ignore
//#else
        runTool yarnTool "webpack-dev-server" __SOURCE_DIRECTORY__ |> ignore
//#endif
    }
    let browser = async {
        do! Async.Sleep 5000
...
let buildDocker tag =
    ...
    runTool "docker" args __SOURCE_DIRECTORY__ |> ignore
...
Target.create "Publish" (fun _ ->
    ...
    runTool "docker" pushArgs __SOURCE_DIRECTORY__ |> ignore
)
...
Target.create "ClusterAuth" (fun _ ->
    ...
    runTool (gcloudTool ()) authArgs __SOURCE_DIRECTORY__ |> ignore
)
...
Target.create "Deploy" (fun _ ->
    runTool (gcloudTool ()) "app deploy --quiet" __SOURCE_DIRECTORY__ |> ignore
)
//#endif
...
Target.create "Configure" (fun args ->
    ...
    let output = runTool herokuTool arguments __SOURCE_DIRECTORY__
    let app = (output.Split '|').[0]
    printfn "app created in %s" (app.Trim())
    let appName = app.[8..(app.IndexOf(".")-1)]
    runTool gitTool "init" __SOURCE_DIRECTORY__ |> ignore
    let gitCmd = sprintf "git:remote --app %s" appName
    runTool herokuTool gitCmd __SOURCE_DIRECTORY__ |> ignore
    runTool herokuTool "buildpacks:set -i 1 https://github.com/heroku/heroku-buildpack-nodejs" __SOURCE_DIRECTORY__ |> ignore
    runTool herokuTool "buildpacks:set -i 2 https://github.com/SAFE-Stack/SAFE-buildpack" __SOURCE_DIRECTORY__ |> ignore
    runTool gitTool "add ." __SOURCE_DIRECTORY__ |> ignore
    runTool gitTool "commit -m initial" __SOURCE_DIRECTORY__ |> ignore
)
...
Target.create "Deploy" (fun _ ->
    let gitTool = platformTool "git" "git.exe"
    runTool gitTool "push heroku master" __SOURCE_DIRECTORY__ |> ignore
    let herokuTool = platformTool "heroku" "heroku.cmd"
    runTool herokuTool "open" __SOURCE_DIRECTORY__ |> ignore
)
//#endif
