                ...
                Common =
                    { args.Common with
                        CustomParams =
                            Some(
                                sprintf
                                    "/p:PackageVersion=%s /p:PackageReleaseNotes=\"%s\""
...
Target.create "Install" (fun _ ->
    ...
    if not result.OK then
        failwithf "`dotnet %s` failed with %O" args result)
...
"Clean" ==> "Pack" ==> "Install" =?> ("Tests", not skipTests)
==> "Push"
==> "Release"
