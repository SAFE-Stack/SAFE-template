...
let waitAndRetry seconds times (func: unit -> 'result) =
    Policy
        .Handle<Exception>()
        .WaitAndRetry(retryCount = times, sleepDurationProvider = (fun _ _ -> TimeSpan.FromSeconds seconds))
        .Execute(func)
    ...
    for pid in getProcessTree pid do
        let proc =
            ...
            with :? ArgumentException ->
                logger.warn (eventX "Can't kill process {pid}: process is not running" >> setField "pid" pid)
                None

        match proc with
        | Some proc when not proc.HasExited ->
            logger.info (eventX "Killing process {pid}" >> setField "pid" pid)
...
let testTemplateBuild templateType =
    ...
    <| fun () ->
        ...
        try
            ...
            logger.info (eventX "Requesting `{url}`" >> setField "url" clientUrl)
            let response = waitAndRetry 3 5 (fun () -> get clientUrl)
            Expect.stringContains response htmlSearchPhrase (sprintf "html fragment not found for %A" templateType)
            extraProc |> Option.iter (fun (_, wait) -> Async.RunSynchronously(wait timeout))
            logger.info (eventX "Requesting `{url}`" >> setField "url" serverUrl)
